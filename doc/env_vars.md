...
| `LRSQL_STMT_GET_MAX_CSV`        | `stmtGetMaxCsv`         | The maximum allowed `limit` value applied to a statement download. If an explicit `limit` value exceeds this value, it will be overridden. As an optional value, if left empty all statements in the LRS will be downloaded.              | (None)                                                                                |
