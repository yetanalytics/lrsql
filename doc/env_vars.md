...
| `LRSQL_JWT_COMMON_SECRET    `     | `jwtCommonSecret`        |  This variable serves as a secret key with which JWT's are signed. If this is unspecified, sql-lrs generates its own key, but when running multiple instances of sql-lrs behind load balancer, they need to accept each others' JWTs, meaning they need to be working from a common key.  This is a secret and should be secured as such.  It should be between 32 and 64 characters.| Not Set |
